...
bool read_file(std::filesystem::path, std::unique_ptr<u8[]>& data, u64&);
...
namespace {
    ...
    void load_shaders()
    {
        ...
        std::wstring defines[]{ L"ELEMENTS_TYPE=1", L"ELEMENTS_TYP#=3"};
...
{
    // NOTE: you can get these model if you're a patreon supported of Quantum Engine.
    //       Use the editor to import the scene and put the 3 models in this location.
    //       You can replace them any model that's available to you.
