...
namespace {
    ...
    util::vector<graphics::light> disable_lights;
...
void generate_lights()
{
    ...
    create_light({ 0, 0.2, 1.f }, {}, graphics::light::point, left_set);
...
void test_lights(f32 dt)
{
    ...
    for (u32 i{ 0 }; i < count; ++i)
    {
        ...
        util::erase_unordered(lights, index);
        disable_lights.emplace_back(light);
    }
    ...
    for (u32 i{ 0 }; i < count; ++i)
    {
        if (!disable_lights.size()) break;
        const u32 index{ (u32)(random() * (disable_lights.size() - 1)) };
        graphics::light light{ disable_lights[index] };
        const game_entity::entity_id id{ light.entity_id() };
        graphics::remove_light(light.get_id(), light.light_set_key());
        remove_game_entity(id);
        light.is_enabled(false);
        util::erase_unordered(disable_lights, index);
    }
