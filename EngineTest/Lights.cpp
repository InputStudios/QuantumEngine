...
void test_lights(f32 dt) {
    ...
    for (u32 i{ 0 }; i < count; ++i) {
        ...
        remove_game_entity(id);
        light.is_enabled(false);
        util::erase_unordered(disabled_lights, index);
    }
    ...
    for (u32 i{ 0 }; i < count; ++i) {
        ...
        create_light(p2, { random() * 3.14f, random() * 3.14f, random() * 3.14f }, random() > 0.5f ? graphics::light::spot : graphics::light::point, left_set);
    }
