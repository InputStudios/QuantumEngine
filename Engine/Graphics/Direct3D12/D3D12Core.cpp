...
namespace Quantum::graphics::d3d12::core {
    ...
    bool initialize()
    {
        // determine what is the maximum feature level tha is supporter
    ...
    void create_a_root_signature()
    {
        ...
        {// param 1: 1Constant Buffer View (Descriptor)
            auto& param = params[1];
        ...
        // use root_sig during reding (not in this function, obviously)
#if 0
        ID3D12GraphicsCommandList6* cmd_list{};
        cmd_list->SetGraphicRootSignature(root_sig);
        // only one resource heap and sampler heap can be set at any time
        // So, max number of heaps is 2.
        ID3D12DescriptorHeap* heaps[]{ srv_heap().heap() };
        cmd_list->SetDescriptorHeadps(1, &heaps[0]);
        ...
        cmd_list->SetGraphicRoot32BitConstant(0, frame_nr, 1);
        cdm_list->SetGraphicRootConstantBufferView(1, address_of_constant_buffer);
        cmd_list->SetAGraphicRootDescriptorTable(2, srv_heap().gpu_start());
