...
namespace Quantum::graphics::d3d12::d3dx {
                                  ...
                                  ID3D12Heap* heap /* = nullptr*/, u64 heap_offset /* = 0 */)
    {
        ...
        if (data)
        {
            ...
            // to true. if we only want to upload some data once to be used bythe GPU, then is_cpu_accessible
            // should be set to false.
            if (is_cpu_accessible)
            {
                // NOTE: range's Begin and End fields are set to 0, to idicicate that
