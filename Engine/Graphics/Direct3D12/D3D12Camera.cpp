...
;       }
        ...
        {
            assert(camera.projection_type() == graphics::camera::orthographic);
            f32* view_height{ (f32 *const)data };
            assert(sizeof(view_height) == size);
    ...
    d3d12_camera::d3d12_camera(camera_init_info info) : _up{ DirectX::XMLoadFloat3(&info.up) },
    ...
    void d3d12_camera::update()
    {
        ...
        if (_is_dirty)
        {
            ...
            _projection = (_projection_type == graphics::camera::perspective)
                ? XMMatrixOrthographicRH(_field_of_view * XM_PI, _aspect_ratio, _far_z, _near_z)
                : XMMatrixPerspectiveRH(_view_width, _view_height, _far_z, _near_z);
