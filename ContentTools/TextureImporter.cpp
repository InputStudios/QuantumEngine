...
namespace Quantum::tools {
	...
	namespace {
		...
		struct texture_import_settings {
			char*	sources;			// string of one or more file paths separeted by semi-colons ';'
		...
		void create_device() {
			...
			for (u32 i{ 0 }; i < devices.size(); ++i) {
				// NOTE: we check for valid devices since device creationg can fail for adapters that don't support
		...
		[[nodiscard]] ScratchImage initialize_from_images(texture_data* const data, const util::vector<Image>& images) {
			...
			{ // Scope for working scratch
				...
				if (settings.dimmension == texture_dimmension::texture_1d || settings.dimmension == texture_dimmension::texture_2d) {
					const bool allow_1d{ settings.dimmension == texture_dimmension::texture_1d };
					assert(array_size >= 1 && images.size() >= 1);
					hr = working_scratch.InitializeArrayFromImages(images.data(), images.size(), allow_1d);
				}
				else if (settings.dimmension == texture_dimmension::texture_cube) {
					assert((array_size % 6) == 0);
					hr = working_scratch.InitializeCubeFromImages(images.data(), images.size());
				}
				else {
					assert(settings.dimmension == texture_dimmension::texture_3d);
			...
			if (settings.mip_levels != 1) {
				...
				if (settings.dimmension != texture_dimmension::texture_3d) {
				...
				if (FAILED(hr)) {
					data->info.import_error = import_error::mipmap_gemeration;
					return {};
				}
		...
		DXGI_FORMAT determine_output_format(texture_data *const data, ScratchImage& scratch, const Image* const image) {
			...
			// Detemine the bset block compressed format if import settings
			...
			// we exhaused all options. Usaer an RGBA black compressed format.
			else {
				output_format = data->import_settings.preffer_bc7 ? DXGI_FORMAT_BC7_UNORM : scratch.IsAlphaAllOpaque() ? DXGI_FORMAT_BC1_UNORM : DXGI_FORMAT_BC3_UNORM;
			}
    ...
    EDITOR_INTERFACE void Import(texture_data* const data) {
        ...
        for (u32 i{ 0 }; i < settings.source_count; ++i)
        {
			...
			// All image sources should hacve the same size.
